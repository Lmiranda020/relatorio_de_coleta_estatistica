...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Kg de Resíduos", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o peso de resíduos gerados em cada centro de custo**
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de coleta, tratamento e destinação de resíduos** entre os centros que geraram os resíduos
        3. **Unidade de medida**: Quilogramas (Kg)
        
        **Instruções específicas:**
        - Inclua apenas resíduos efetivamente gerados no período de referência
        - Associe cada valor ao centro de custo que gerou o resíduo
        - Deixe em branco ou zero se não houver geração de resíduos para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 120 Kg de resíduos
        - Centro Cirúrgico: 80 Kg de resíduos
        - Ambulatório: 50 Kg de resíduos
        
        Conclusão do exemplo:
        No período analisado, o total de resíduos será utilizado para calcular o **rateio proporcional dos custos de gestão de resíduos** entre os diferentes centros de custo.
        
        ⚠️ **Importante**: Registre apenas os resíduos efetivamente gerados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Kg de Resíduos"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Kg de Resíduos", total_kg)

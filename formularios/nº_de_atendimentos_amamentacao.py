...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Atendimentos (Amamentação)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de atendimentos realizados para orientação e suporte à amamentação**, associando cada atendimento ao centro de custo do paciente beneficiado.
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais do serviço de amamentação** entre os centros que se beneficiaram do atendimento.
        3. **Unidade de medida**: Número de atendimentos.
        
        **Instruções específicas:**
        - Cada atendimento realizado corresponde a **uma unidade de registro**.
        - Associe cada atendimento ao **centro de custo do paciente**.
        - Deixe em branco ou zero se não houver atendimentos para determinado centro de custo.
        
        **Exemplo de cálculo:**
        - Dia 04/06/2022:
            - Paciente A (UTI Neonatal) → 1 atendimento
            - Paciente B (Enfermaria Pediátrica) → 1 atendimento
        - Dia 10/06/2022:
            - Paciente C (Clínica Médica) → 1 atendimento
        - Total do mês:
            - UTI Neonatal: 1 atendimento
            - Enfermaria Pediátrica: 1 atendimento
            - Clínica Médica: 1 atendimento
        
        ✅ **Conclusão do exemplo**:
        O total de atendimentos será utilizado para calcular o **rateio proporcional dos custos operacionais do serviço de amamentação** entre os diferentes centros de custo beneficiados.
    ...
    try:
        ...
        nome_formulario = "Nº de atendimentos (amamentação)"
        ...
        ponderacao = "Nº de atendimentos (amamentação)"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de atendimentos (amamentação)", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Óbitos", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de óbitos ocorridos**, associando cada ocorrência ao centro de custo em que o paciente estava internado
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos relacionados à gestão de óbitos** (ex.: serviços administrativos, necrotério, SCIH, comunicação familiar) entre os setores envolvidos
        3. **Unidade de medida**: Número de óbitos (cada paciente falecido corresponde a uma ocorrência)
        
        **Instruções específicas:**
        - Registre cada óbito no centro de custo onde o paciente estava no momento do evento
        - Se houver mais de um óbito no mesmo setor, registre o total do período
        - Não registrar duplicações (um óbito = uma ocorrência)
        - Deixe em branco ou zero se não houver registros de óbito no setor no período
        
        **Exemplo de cálculo:**
        - Dia 05/06/2022: 1 óbito na UTI → **1 ocorrência**
        - Dia 09/06/2022: 2 óbitos na Clínica Médica → **2 ocorrências**
        - Dia 12/06/2022: 1 óbito no Pronto-Socorro → **1 ocorrência**
        
        - Total do mês:
            - UTI: 1 ocorrência
            - Clínica Médica: 2 ocorrências
            - Pronto-Socorro: 1 ocorrência
        
        Conclusão do exemplo:
        O total de óbitos será utilizado para calcular o **rateio proporcional dos custos relacionados à gestão e suporte de óbitos** entre os centros de custo.
        
        ⚠️ **Importante**: Registre apenas os óbitos efetivamente ocorridos no período de referência.
        """)
    ...
    try:
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Óbitos", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Curativos Realizados", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de curativos realizados**, associando cada atendimento ao centro de custo do paciente
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais do setor de curativos** entre os centros que se beneficiaram do serviço
        3. **Unidade de medida**: Número de curativos realizados (cada procedimento corresponde a **1 atendimento**)
        
        **Instruções específicas:**
        - Cada curativo realizado deve ser contabilizado como **1 atendimento**
        - É essencial registrar o **centro de custo de origem do paciente** (ex.: Clínica Médica, UTI, Pronto-Socorro)
        - Se o mesmo paciente necessitar de curativos em dias diferentes, registre cada atendimento separadamente
        - Deixe em branco ou zero se não houver curativos para determinado centro de custo
        
        **Exemplo de cálculo:**
        - Dia 08/06/2022:
            - Paciente da Clínica Médica → 2 curativos → **2 atendimentos**
            - Paciente da UTI → 1 curativo → **1 atendimento**
        - Dia 09/06/2022:
            - Paciente do Pronto-Socorro → 3 curativos → **3 atendimentos**
        - Total do mês:
        ...
        Os números registrados serão utilizados para calcular o **rateio proporcional dos custos do setor responsável pelos curativos** entre os centros de custo atendidos.
        
        ⚠️ **Importante**: Registre apenas os curativos efetivamente realizados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Curativos Realizados"
        ...
        ponderacao = "Nº de Curativos Realizados"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Curativos Realizadoss", total_quantidade)

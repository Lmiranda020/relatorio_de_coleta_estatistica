...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Cópias + Impressões + Encadernações", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade de cópias, impressões e encadernações** realizadas em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de impressão e encadernação** entre os setores
        3. **Unidade de medida**: Número de cópias/impressões/encadernações
        
        **Instruções específicas:**
        - Inclua todas as cópias, impressões e encadernações realizadas internamente ou por fornecedores
        - Associe cada atividade ao centro de custo que utilizou o serviço
        - Deixe em branco ou zero se não houver serviços realizados para o centro de custo
        
        **Exemplos:**
        - Recepção: 200 cópias e 50 encadernações
        - Ambulatório: 150 impressões
        - Administração: 100 cópias e 30 encadernações
        
        Conclusão do exemplo:
        No período analisado, o total de cópias, impressões e encadernações será utilizado para calcular o **rateio proporcional dos custos de material e serviços gráficos** entre os diferentes centros de custo.
        
        ⚠️ **Importante**: Certifique-se de registrar apenas os serviços efetivamente realizados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Cópias + Impressões + Encardenações"
        ...
        ponderacao = "Nº de Cópias + Impressões + Encadernações"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Cópias + Impressões + Encadernações", total_quantidade)

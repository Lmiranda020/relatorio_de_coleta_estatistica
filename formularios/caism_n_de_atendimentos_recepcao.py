...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - CAISM - Nº de Atendimentos Recepção", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de atendimentos realizados na recepção do CAISM**, associando cada atendimento ao centro de custo do paciente
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais da recepção do CAISM** entre os centros que utilizaram o serviço
        3. **Unidade de medida**: Número de atendimentos (cada paciente atendido = 1 ocorrência)
        
        **Instruções específicas:**
        - Cada atendimento realizado na recepção deve ser contabilizado como **1 atendimento**
        ...
        **Exemplo de cálculo:**
        - Dia 05/06/2022:
            - Pacientes da Clínica Obstétrica → 8 atendimentos
            - Pacientes do Pronto Socorro → 5 atendimentos
        - Dia 06/06/2022:
            - Pacientes da UTI → 2 atendimentos
        - Total do mês:
            - Clínica Obstétrica: 8 atendimentos
            - Pronto Socorro: 5 atendimentos
            - UTI: 2 atendimentos
        
        Conclusão do exemplo:
        Os números registrados permitirão calcular o **rateio proporcional dos custos da recepção do CAISM** entre os centros de custo atendidos.
    ...
    try:
        ...
        nome_formulario = "CAISM - N° de Atendimentos Recepção"
        ...
        ponderacao = "CAISM - N° de Atendimentos Recepção"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de CAISM - N° de Atendimentos Recepção", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Quantidade de Pacientes (OPM)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de pacientes distintos atendidos pelo setor de OPM** no período de referência.
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais** do setor de OPM com base na demanda de pacientes atendidos.
        3. **Unidade de medida**: Quantidade de pacientes únicos.
        
        **Instruções específicas:**
        - Cada paciente conta **uma única vez**, mesmo que tenha recebido mais de uma peça ou feito múltiplas visitas no período.
        - Se o paciente retornou em outro dia para nova solicitação, mas dentro do mesmo período de cálculo, ainda assim conta apenas uma vez.
        - Associe o paciente ao centro de custo solicitante no momento do atendimento.
        
        **Exemplo de cálculo:**
        - Dia 03/06/2022: Paciente A recebeu 2 próteses → **1 paciente**.
        - Dia 04/06/2022: Paciente B recebeu 1 órtese → **1 paciente**.
        - Total do mês: **2 pacientes atendidos**. (2 peças)
        
        Conclusão do exemplo:
        O número de pacientes será usado para **ratear os custos do setor de OPM** proporcionalmente entre os centros de custo atendidos.
        
        ⚠️ **Importante**: Considere apenas pacientes efetivamente atendidos pelo setor, não inclua solicitações não realizadas.
        """)
    ...
    try:
        ...
        nome_formulario = "Quantidade de pacientes - OPM"
        ...
        ponderacao = "Quantidade de pacientes - OPM"
        ...
        if not df_resultado.empty:
            ...
            total_horas = df_resultado['Quantidade'].sum()
            ...
            with col3:
                st.metric("Quantidade Total", total_horas)

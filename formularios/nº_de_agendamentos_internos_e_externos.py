...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Agendamentos Internos e Externos", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade de agendamentos internos e externos** realizados em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos administrativos da regulação** para os centros que receberam os agendamentos
        3. **Unidade de medida**: Número de agendamentos
        
        **Instruções específicas:**
        - Agendamentos internos: pacientes atendidos dentro da própria unidade
        - Agendamentos externos: pacientes encaminhados para outros serviços ou unidades
        - Associe cada agendamento ao centro de custo que efetivamente utilizará o serviço (ex.: clínica médica, ambulatório, UTI)
        - A regulação funciona como um centro de custo administrativo/auxiliar; os custos da regulação serão **rateados proporcionalmente** entre os centros que receberam os agendamentos
        - Deixe em branco ou zero se não houver agendamentos para o centro de custo
        
        **Exemplos:**
        - Regulacão realizou 50 agendamentos para Clínica Médica: esses custos serão rateados para a Clínica Médica
        - Regulacão realizou 30 agendamentos para Ambulatório: esses custos serão rateados para o Ambulatório
        - Total de agendamentos internos e externos serão contabilizados para o cálculo do rateio
        
        Conclusão do exemplo:
        No período analisado, os custos da regulação serão distribuídos proporcionalmente aos centros que receberam os agendamentos, garantindo que cada setor absorva a parte correspondente do custo administrativo.
        
        ⚠️ **Importante**: Registre apenas agendamentos efetivamente realizados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Agendamentos Internos e Externos"
        ...
        ponderacao = "Nº de Agendamentos Internos e Externos"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Agendamentos Internos e Externos", total_quantidade)

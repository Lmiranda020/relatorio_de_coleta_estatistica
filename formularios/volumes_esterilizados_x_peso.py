...
def render_form(competencia):
    ...
    """
    with st.expander("📋 Instruções - CME (Central de Material Esterilizado)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o volume de materiais esterilizados** processados pela CME para cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de esterilização** entre os centros que utilizaram o serviço
        3. **Unidade de medida**: Volume x Peso dos materiais esterilizados
        
        **Instruções específicas:**
        - Inclua apenas materiais efetivamente processados pela Central de Material Esterilizado
        - Associe cada volume ao centro de custo que utilizou os materiais esterilizados
        - Deixe em branco ou zero se não houver volume processado para o centro de custo
        
        **Exemplos:**
        - Instrumentais cirúrgicos do Centro Cirúrgico: 150 unidades
        - Materiais da UTI: 80 unidades
        - Equipamentos do Ambulatório: 25 unidades
        
        Conclusão do exemplo:
        No período analisado, o total de volumes esterilizados será utilizado para calcular o **rateio proporcional dos custos da CME** entre os diferentes centros de custo que utilizaram o serviço.
        
        ⚠️ **Importante**: Registre apenas volumes efetivamente esterilizados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Volumes Esterilizados x Peso"
        ...
        ponderacao = "Volumes Esterilizados x Peso"
        ...
        if form_key not in st.session_state:
            ...
            if dados_salvos:
                for codigo_cc, dados in dados_salvos.items():
        ...
        for idx, row in centros_aplicaveis.iterrows():
            ...
            entrada = st.text_input(
                ...
                value=valor_default,  # ✅ Valor salvo ou "0"
                # ... resto igual
                key=field_key  # ✅ Adicionar key
            )

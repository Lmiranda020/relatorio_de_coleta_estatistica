...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - RC_Classificação_de_Risco", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Ponderação**: Nº de Atendimentos Classificacao de Risco
        2. **Objetivo**: Registrar os atendimentos classificados por risco
        3. **Unidade de medida**: Atendimentos (quantidade de atendimentos)

        **Instruções específicas:**
        - Informe a quantidade de atendimentos para cada centro de custo listado
        - Use valores inteiros (ex: 8, 40, 120)
        - Deixe em branco ou zero se não houver utilização

        **Exemplo:** 
        Após passar pela **classificação de risco**, o paciente José foi identificado como **risco baixo** e encaminhado para atendimento na **Clínica Médica**.  
        Neste caso, o atendimento deve ser registrado no **centro de custo correspondente à Clínica Médica**, pois é onde o serviço foi efetivamente prestado.
    ...
    try:
        ...
        nome_formulario = "Classificação de Risco"
        ...
        ponderacao = "Nº de Atendimentos Classificacao de Risco"

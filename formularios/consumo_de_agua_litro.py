...
def render_form(competencia):
    ...
    # Caixa de instruções
    with st.expander("📋 Instruções - Consumo de Água (Litro)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o consumo de água em litros** para cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de água** entre os setores
        3. **Unidade de medida**: Litros (L)
        
        **Instruções específicas:**
        - Inclua apenas o consumo efetivamente medido ou estimado para cada centro de custo
        - Associe cada valor ao centro de custo correspondente
        - Deixe em branco ou zero se não houver consumo para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 12.000 L
        - Centro Cirúrgico: 8.500 L
        - Ambulatório: 5.000 L
        
        Conclusão do exemplo:
        No período analisado, o total de água consumida será utilizado para calcular o **rateio proporcional dos custos de água** entre os diferentes centros de custo.
        
        ⚠️ **Importante**: Certifique-se de registrar apenas o consumo efetivo no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Consumo de Água (Litro)"
        ...
        ponderacao = "Consumo de Água (litros)"
        ...
        for idx, row in centros_aplicaveis.iterrows():
            ...
            entrada = st.text_input(
                ...
                value="0",
                placeholder="ex: 40, 75, 120",
                key=f"{nome_formulario}_{codigo_cc}_{competencia}_{unidade_selecionada}",
                help="Digite apenas números inteiros (ex: 40, 75, 120)"
            )
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Consumo", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Itens Movimentados (Farmácia Oncologia)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de movimentações de itens realizadas pela Farmácia Oncologia**, associando cada movimentação ao centro de custo solicitante.
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais da Farmácia Oncologia** entre os centros que se beneficiaram do serviço de dispensa.
        ...
        **Exemplo de cálculo:**
        - Dia 04/06/2022:
            - Requisição do Centro Oncológico: 1 unidade de **Fentanil** e 1 unidade de **Cisplatina** → **1 movimentação**
            - Requisição da UTI: 1 unidade de **Midazolam** → **1 movimentação**
        - Dia 10/06/2022:
            - Requisição da Clínica Médica: 1 unidade de **Paracetamol** e 1 unidade de **Amoxicilina** → **1 movimentação**
        - Total do mês:
            - Centro Oncológico: 1 movimentação
            - UTI: 1 movimentação
            - Clínica Médica: 1 movimentação
        
        ✅ **Conclusão do exemplo**:
        O total de movimentações será utilizado para calcular o **rateio proporcional dos custos operacionais da Farmácia Oncologia** entre os diferentes centros de custo beneficiados.
        
        ⚠️ **Importante**: Registre apenas as movimentações efetivamente realizadas no período de referência e associe corretamente ao centro de custo solicitante.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Itens Movimentados - Farmácia Oncologia"
        ...
        ponderacao = "Nº de Itens Movimentados - Farmácia Oncologia"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Itens Movimentados - Farmácia Oncologia", total_quantidade)

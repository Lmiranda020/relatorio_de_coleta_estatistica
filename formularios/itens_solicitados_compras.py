...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Itens Solicitados (Compras)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de itens solicitados ao setor de Compras**, discriminando por centro de custo de origem
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais do setor de Compras** entre os centros que demandaram aquisições
        3. **Unidade de medida**: Número de itens solicitados (cada linha de pedido corresponde a um item, independente da quantidade comprada)
        
        **Instruções específicas:**
        - Cada item corresponde a um produto ou serviço solicitado ao setor de Compras
        - É fundamental associar o item ao **centro de custo que originou a solicitação**
        - Se houver múltiplos pedidos do mesmo item em dias diferentes, registre cada solicitação separadamente
        - Deixe em branco ou zero se não houver solicitações de determinado centro de custo
        
        **Exemplo de cálculo:**
        - Dia 05/06/2022:
            - Centro Cirúrgico solicitou: 1 bisturi, 1 caixa de luvas → **2 itens**
            - UTI solicitou: 1 monitor multiparamétrico → **1 item**
        - Dia 06/06/2022:
            - Clínica Médica solicitou: 1 cadeira de rodas, 1 bomba de infusão, 1 oxímetro → **3 itens**
        - Total do mês:
            - Centro Cirúrgico: 2 itens
            - UTI: 1 item
            - Clínica Médica: 3 itens
        
        Conclusão do exemplo:
        Os valores registrados permitirão calcular o **rateio proporcional dos custos do setor de Compras** entre os centros de custo que realizaram solicitações.
        
        ⚠️ **Importante**: Registre apenas os itens efetivamente solicitados no período e associe corretamente ao centro de custo solicitante.
        """)
    ...
    try:
        ...
        nome_formulario = "Itens Solicitados - Compras"
        ...
        ponderacao = "Itens Solicitados - Compras"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Itens Solicitados - Compras", total_quantidade)

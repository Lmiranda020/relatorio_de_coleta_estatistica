...
def render_form(competencia):
    """
    Renderiza o formulário % Consumo Gerador
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - % Consumo Gerador", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a porcentagem de consumo do gerador atribuída a cada centro de custo**
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de energia gerada pelo gerador** entre os centros que utilizaram o serviço
        3. **Unidade de medida**: Porcentagem (%)
        
        **Instruções específicas:**
        - A porcentagem deve refletir o quanto cada centro de custo utilizou a energia gerada pelo equipamento
        - Associe cada valor ao centro de custo correspondente
        - Deixe em branco ou zero se o centro de custo não utilizou energia do gerador
        - **Digite apenas o número (ex: 50 para 50%, 30 para 30%)**
        
        **Exemplos:**
        - UTI Adulto: 50% de consumo → **Digite: 50**
        - Centro Cirúrgico: 30% de consumo → **Digite: 30**
        - Ambulatório: 20% de consumo → **Digite: 20**
        
        Conclusão do exemplo:
        No período analisado, os dados de consumo serão utilizados para calcular o **rateio proporcional dos custos de combustível e operação do gerador** entre os diferentes centros de custo.
        
        ⚠️ **Importante**: Certifique-se de registrar apenas a participação efetiva de cada centro de custo no consumo do gerador no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "% CONSUMO GERADOR"
        ...
        ponderacao = "% de Utilização Gerador"
        ...
        if calcular or st.session_state[validation_key].get('calculado', False):
            ...
            with col3:
                st.metric("Total % Consumo Gerador", f"{validation_state['total']}%")

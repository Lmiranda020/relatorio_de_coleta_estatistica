...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Itens Movimentados (Almoxarifado Unitarização)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de movimentações de itens realizadas pelo Almoxarifado de Unitarização**, associando cada movimentação ao centro de custo solicitante.
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais do Almoxarifado de Unitarização** entre os centros que se beneficiaram do serviço.
        ...
        **Exemplo de cálculo:**
        - Dia 04/06/2022:
            - Requisição do Centro Cirúrgico: 1 unidade de **Luvas Cirúrgicas** e 1 unidade de **Máscaras** → **1 movimentação**
            - Requisição da UTI: 1 unidade de **Seringa** → **1 movimentação**
        - Dia 10/06/2022:
            - Requisição da Clínica Médica: 1 unidade de **Gaze** e 1 unidade de **Saco de Soro** → **1 movimentação**
        - Total do mês:
        ...
        O total de movimentações será utilizado para calcular o **rateio proporcional dos custos operacionais do Almoxarifado de Unitarização** entre os diferentes centros de custo beneficiados.
    ...
    try:
        ...
        nome_formulario = "Nº de Itens Movimentados - Almoxarifado Unitarização"
        ...
        ponderacao = "Nº de Itens Movimentados - Almoxarifado Unitarização"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Itens Movimentados - Almoxarifado Unitarização", total_quantidade)

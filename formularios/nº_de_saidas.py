...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Saídas", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de saídas de pacientes**, associando cada ocorrência ao centro de custo de origem do paciente
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais de internação e atendimento** entre os centros que receberam pacientes
        3. **Unidade de medida**: Número de saídas de pacientes (cada saída = 1 ocorrência)
        
        **Instruções específicas:**
        - Cada paciente que recebe alta, transferência ou saída temporária deve ser contabilizado como **1 saída**
        - Associe sempre ao **centro de custo do setor de origem** do paciente
        - Se houver múltiplas saídas temporárias no período, registre cada ocorrência separadamente
        - Deixe em branco ou zero se não houver saídas no período
        
        **Exemplo de cálculo:**
        - Dia 05/06/2022:
            - 3 saídas da Clínica Médica → 3 ocorrências
            - 2 saídas da UTI → 2 ocorrências
        - Dia 07/06/2022:
            - 1 saída do Pronto-Socorro → 1 ocorrência
        - Total do mês:
            - Clínica Médica: 3 saídas
            - UTI: 2 saídas
            - Pronto-Socorro: 1 saída
        
        Conclusão do exemplo:
        Os números registrados serão utilizados para calcular o **rateio proporcional dos custos de internação, hotelaria e atendimento** entre os centros de custo.
        
        ⚠️ **Importante**: Registre apenas as saídas efetivamente realizadas no período de referência e associe corretamente ao centro de custo de origem.
        """)
    ...
    try:
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Saídas", total_quantidade)

...
def render_form(competencia):
    ...
    # Caixa de instruções
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Contratos Gerenciados", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de contratos ativos ou gerenciados pela unidade/centro de custo** no período de referência
        2. **Objetivo**: Auxiliar na **avaliação da carga administrativa e no rateio dos custos relacionados à gestão contratual**
        3. **Unidade de medida**: Quantidade de contratos
        
        **Instruções específicas:**
        - Considere contratos de serviços terceirizados, manutenção, fornecimento de insumos, tecnologia, vigilância, limpeza, entre outros
        - Registre apenas contratos **efetivamente geridos/ativos** no período
        - Se um contrato for compartilhado entre vários setores, registre-o no setor principal responsável pela gestão
        - Deixe em branco ou zero se não houver contratos sob gestão no período
        
        **Exemplo de cálculo:**
        - Contrato de vigilância patrimonial → 1 contrato
        - Contrato de limpeza terceirizada → 1 contrato
        - Contrato de manutenção de equipamentos → 1 contrato
        - Total do mês: **3 contratos gerenciados**
        
        Conclusão do exemplo:
        O número de contratos gerenciados será utilizado para medir a **complexidade administrativa** e apoiar o **rateio proporcional dos custos de gestão contratual** entre as áreas envolvidas.
        
        ⚠️ **Importante**: Registre apenas os contratos efetivamente ativos no período, evitando duplicidades.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Contratos Gerenciados"
        ...
        ponderacao = "Nº de Contratos Gerenciados"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Contratos Gerenciados", total_quantidade)

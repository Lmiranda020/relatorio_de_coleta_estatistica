...
def render_form(competencia):
    """
    Renderiza o formulário com suporte a valores decimais e validações melhoradas
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - % Atuação CME (Central de Material e Esterilização)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a porcentagem de atuação da CME** em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos da Central de Material e Esterilização (CME)** entre os setores
        3. **Unidade de medida**: Porcentagem (%)
        
        **Instruções específicas:**
        - A porcentagem deve refletir o quanto a CME contribui para o atendimento de cada centro de custo
        - Associe cada valor ao centro de custo correspondente
        - Deixe em branco ou zero se a CME não atua naquele centro de custo
        - **Digite apenas o número (ex: 80 para 80%, 15.5 para 15.5%)**
        - **Aceita valores decimais usando ponto ou vírgula**
        
        **Exemplos:**
        - Centro Cirúrgico: 80% de atuação da CME → **Digite: 80**
        - UTI Adulto: 15.5% de atuação da CME → **Digite: 15.5 ou 15,5**
        - Ambulatório: 5% de atuação da CME → **Digite: 5**
        
        Conclusão do exemplo:
        No período analisado, os dados de atuação serão utilizados para calcular o **rateio proporcional dos custos da CME** entre os diferentes centros de custo, de acordo com a participação efetiva em cada setor.
        
        ⚠️ **Importante**: Certifique-se de registrar apenas a atuação efetiva da CME no período de referência.
        """)
    ...
    try:
        ...
        if calcular or st.session_state[validation_key].get('calculado', False):
            ...
            with col3:
                st.metric("Total % Atuação CME", f"{validation_state['total']}%")

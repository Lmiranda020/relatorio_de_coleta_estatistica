...
def render_form(competencia):
    ...
    #     )
    #     st.session_state["scroll_top"] = False  # reseta para não repetir sempre
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de O.S. da Tecnologia da Informação", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade de Ordens de Serviço (O.S.) de TI** realizadas no período para cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio de custos de serviços de TI terceirizados** para cada área
        3. **Unidade de medida**: Quantidade de O.S.
        
        **Instruções específicas:**
        - Inclua apenas serviços de TI contratados externamente (de terceiros)
        - Associe cada O.S. ao centro de custo que utilizou o serviço
        - Deixe em branco ou zero se não houver O.S. para o centro de custo
        
        **Exemplos:**
        - Recepção: 1 O.S. de manutenção de computadores (custo do serviço: R$ 500)
        - Centro Cirúrgico: 2 O.S. de suporte a sistemas digitais (custo total: R$ 1.200)
        - Ambulatório: 1 O.S. de instalação de software clínico (custo: R$ 300)
        
        Conclusão do exemplo:
        Cada centro de custo terá seu **custo proporcional de TI** registrado: Recepção R$ 500, Centro Cirúrgico R$ 1.200 e Ambulatório R$ 300. Esses dados permitem calcular o rateio correto dos custos de TI entre os setores.
        
        ⚠️ **Importante**: Certifique-se de que os valores correspondem apenas a serviços contratados externamente e efetivamente realizados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de O.S. da Tecnologia da Informação"
        ...
        ponderacao = "Nº O.S. Tecnologia da Informação"
        ...
        if not df_resultado.empty:
            ...
            total_ordem = df_resultado['Quantidade'].sum()
            ...
            with col3:
                st.metric("Total de O.S", total_ordem)

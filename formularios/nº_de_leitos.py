...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Leitos", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número total de leitos disponíveis** em cada centro de custo (enfermaria, UTI, pronto socorro, etc.)
        2. **Objetivo**: Auxiliar na **análise de capacidade instalada e no rateio dos custos fixos** relacionados à estrutura hospitalar
        3. **Unidade de medida**: Número de leitos (quantitativo físico disponível, independente de ocupação)
        
        **Instruções específicas:**
        - Considere apenas os leitos **existentes e habilitados para uso**, mesmo que estejam temporariamente desocupados
        - Leitos inativos ou interditados não devem ser contabilizados
        - Associe cada valor ao centro de custo correspondente
        - Deixe em branco ou zero se o centro não possuir leitos
        
        **Exemplo de preenchimento:**
        - Enfermaria Adulto: 40 leitos
        - UTI Adulto: 10 leitos
        - UTI Neonatal: 6 leitos
        - Pronto Socorro: 8 leitos de observação
        
        Conclusão do exemplo:
        O total de leitos informado será utilizado para calcular o **custo médio por leito** e para apoiar a **distribuição proporcional dos custos de hotelaria e infraestrutura** entre os centros assistenciais.
        
        ⚠️ **Importante**: Mantenha os números atualizados conforme ampliações, reformas ou alterações na capacidade instalada.
        """)
    ...
    try:
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Leitos", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Kms Rodados", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quilometragem percorrida (Kms) pelos veículos** em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de transporte e manutenção veicular** entre os centros que utilizaram os veículos
        
        **Por que preencher este formulário:**
        - Algumas unidades não possuem transporte próprio e solicitam o serviço a outras unidades (ex.: núcleo administrativo)
        - Outras unidades possuem frota própria e registram a utilização interna
        - Quando uma unidade solicita transporte a outra, **o centro de custo registrado será da unidade solicitante**
        - Registrando os Kms percorridos, será possível **ratear proporcionalmente os custos de transporte e manutenção** entre o centro que executou o serviço e a unidade solicitante
        
        3. **Unidade de medida**: Quilômetros (Km)
        
        **Instruções específicas:**
        - Inclua apenas os Kms efetivamente percorridos no período de referência
        - Associe cada valor ao centro de custo que utilizou o veículo (a unidade solicitante)
        - Deixe em branco ou zero se não houver utilização do veículo para o centro de custo
        
        **Exemplos:**
        - UBS solicita transporte ao núcleo administrativo: 50 Km (registrados no centro de custo da UBS)
        - Transporte de documentos pelo núcleo administrativo para Ambulatório: 30 Km (registrados no centro de custo do Ambulatório)
        - Transporte interno de materiais administrativos: 20 Km (registrados no centro de custo que solicitou)
        
        Conclusão do exemplo:
        No período analisado, o total de quilômetros percorridos será utilizado para calcular o **rateio proporcional dos custos de transporte e manutenção** entre os diferentes centros de custo, considerando quem solicitou e quem executou o serviço.
        
        ⚠️ **Importante**: Registre apenas os quilômetros efetivamente rodados pelos veículos no período de referência.
        """)
    ...
    try:
        ...
        if not df_resultado.empty:
            ...
            total_km = df_resultado['Quantidade'].sum()
            ...
            with col3:
                st.metric("Total de Kms Rodados", total_km)

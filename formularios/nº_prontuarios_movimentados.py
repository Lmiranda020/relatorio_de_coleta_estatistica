...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Prontuários Movimentados", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de prontuários movimentados**, associando cada solicitação ao centro de custo que requisitou o prontuário
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais do setor de prontuários/arquivo** entre os centros que utilizam o serviço
        3. **Unidade de medida**: Número de movimentações de prontuários (cada movimentação = 1 solicitação atendida)
        
        **Instruções específicas:**
        - Cada retirada, consulta ou devolução de prontuário conta como **1 movimentação**
        - Sempre associe a movimentação ao **centro de custo solicitante**
        - Se o mesmo prontuário for movimentado várias vezes no período, registre cada movimentação separadamente
        - Deixe em branco ou zero se não houver movimentações para determinado centro de custo
        
        **Exemplo de cálculo:**
        - Dia 12/06/2022:
            - Prontuário de paciente solicitado pela Clínica Médica → **1 movimentação**
            - Prontuário de paciente solicitado pela UTI → **1 movimentação**
        - Dia 14/06/2022:
            - Prontuário solicitado e devolvido pelo Pronto-Socorro → **2 movimentações**
        - Total do mês:
            - Clínica Médica: 1 movimentação
            - UTI: 1 movimentação
            - Pronto-Socorro: 2 movimentações
        
        Conclusão do exemplo:
        O total de movimentações será utilizado para calcular o **rateio proporcional dos custos do setor de prontuários** entre os centros que utilizaram o serviço.
    ...
    try:
        ...
        nome_formulario = "Nº Prontuários Movimentados"
        ...
        ponderacao = "Nº Prontuários Movimentados"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº Prontuários Movimentados", total_quantidade)

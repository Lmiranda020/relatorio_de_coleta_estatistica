...
def render_form(competencia):
    """
    Renderiza o formulário % de Resíduos
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - % de Resíduos", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a porcentagem de geração de resíduos atribuída a cada centro de custo**
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de gerenciamento de resíduos** entre os centros que geraram os resíduos
        3. **Unidade de medida**: Porcentagem (%)
        
        **Instruções específicas:**
        - A porcentagem deve refletir o quanto cada centro de custo contribuiu para a geração total de resíduos
        - Associe cada valor ao centro de custo correspondente
        - Deixe em branco ou zero se o centro de custo não gerou resíduos significativos
        - **Digite apenas o número (ex: 40 para 40%, 35.5 para 35.5%)**
        
        **Exemplos:**
        - Centro Cirúrgico: 40% dos resíduos → **Digite: 40**
        - UTI Adulto: 35.5% dos resíduos → **Digite: 35.5**
        - Ambulatório: 24.5% dos resíduos → **Digite: 24.5**
        
        Conclusão do exemplo:
        No período analisado, os dados de geração de resíduos serão utilizados para calcular o **rateio proporcional dos custos de coleta, tratamento e destinação dos resíduos** entre os diferentes centros de custo.
        
        ⚠️ **Importante**: Certifique-se de registrar apenas a participação efetiva de cada centro de custo na geração de resíduos no período de referência.
        """)
    ...
    try:
        ...
        if calcular or st.session_state[validation_key].get('calculado', False):
            ...
            with col3:
                st.metric("Total % de Resíduos", f"{validation_state['total']}%")

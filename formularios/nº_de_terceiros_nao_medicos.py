...
def render_form(competencia):
    """
    Renderiza o formulário Nº de Terceiros (Não Médicos)"
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Terceiros (Não Médicos)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade des profissionais terceirizados** que atuam na unidade, excluindo médicos
        2. **Objetivo**: Auxiliar no **rateio de custos** por profissionais não médicos em cada centro de custo
        3. **Unidade de medida**: Número de pessoas
        
        **Instruções específicas:**
        - Inclua todos os profissionais terceirizados, como enfermagem, técnico de laboratório, serviços gerais, administrativos, segurança, etc.
        - Não inclua médicos ou profissionais próprios da unidade
        - Preencha cada centro de custo separadamente
        - Deixe em branco ou zero se não houver terceirizados no centro de custo
        
        **Exemplos:**
        - UTI Adulto: 5 terceirizados (enfermeiros e técnicos)
        - Centro Cirúrgico: 3 terceirizados (instrumentadores e serviços gerais)
        - Ambulatório: 2 terceirizados (recepção e limpeza)
        
        Conclusão do exemplo:
        No período analisado, o total de terceirizados não médicos somou 10 profissionais, distribuídos entre os centros de custo. Esses dados serão utilizados para calcular o rateio proporcional dos custos de pessoal entre os diferentes setores.
        
        ⚠️ **Importante**: Confirme que os números correspondem apenas a profissionais terceirizados e ativos no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Terceiros (Não Médicos)"
        ...
        ponderacao = "Nº de Terceiros (Não Médicos)"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Funcionários", total_quantidade)

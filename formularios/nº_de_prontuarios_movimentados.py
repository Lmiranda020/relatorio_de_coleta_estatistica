...
def render_form(competencia):
    ...
    with st.expander("📋 Instruções - Nº de Prontuários Movimentados", expanded=False):
        ...
        1. **Informe a quantidade de prontuários movimentados** (consultados, arquivados ou transferidos) em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos do setor de arquivo e gestão de prontuários** entre os centros que se beneficiaram do serviço
        3. **Unidade de medida**: Número de prontuários
        
        **Instruções específicas:**
        - Inclua apenas prontuários efetivamente movimentados no período de referência
        - Associe cada valor ao centro de custo que utilizou o serviço
        - Deixe em branco ou zero se não houver movimentação de prontuários para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 120 prontuários movimentados
        - Enfermaria Pediátrica: 80 prontuários movimentados
        - Centro Cirúrgico: 50 prontuários movimentados
        
        Conclusão do exemplo:
        No período analisado, o total de prontuários movimentados será utilizado para calcular o **rateio proporcional dos custos do setor de arquivo e gestão de prontuários** entre os diferentes centros de custo beneficiados.
        
        ⚠️ **Importante**: Registre apenas os prontuários efetivamente movimentados no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Prontuários Movimentados"
        ...
        ponderacao = "Nº de Prontuários Movimentados"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Prontuários Movimentados", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Itens Movimentados (Farmácia Ambulatório)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de movimentações de itens realizadas pela Farmácia do Ambulatório**, associando cada movimentação ao centro de custo solicitante.
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais da Farmácia do Ambulatório** entre os centros que se beneficiaram do serviço.
        ...
        **Instruções específicas:**
        - Cada movimentação corresponde a uma requisição atendida pelo setor, mesmo que contenha múltiplos itens.
        ...
        **Exemplo de cálculo:**
        - Dia 04/06/2022:
            - Requisição da Clínica Médica: 1 unidade de **Dipirona** e 1 unidade de **Omeprazol** → **1 movimentação**
            - Requisição da Pediatria: 1 unidade de **Paracetamol** → **1 movimentação**
        - Dia 10/06/2022:
            - Requisição da Ginecologia: 1 unidade de **Ibuprofeno** e 1 unidade de **Vitamina D** → **1 movimentação**
        - Total do mês:
            - Clínica Médica: 1 movimentação
            - Pediatria: 1 movimentação
            - Ginecologia: 1 movimentação
        
        ✅ **Conclusão do exemplo**:
        O total de movimentações será utilizado para calcular o **rateio proporcional dos custos operacionais da Farmácia do Ambulatório** entre os diferentes centros de custo beneficiados.
    ...
    try:
        ...
        nome_formulario = "Nº itens movimentados - Farmácia Ambulatório"
        ...
        ponderacao = "Nº itens movimentados - Farmácia Ambulatório"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº itens movimentados - Farmácia Ambulatório", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Kg de Roupa Suja (Hotelaria)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o peso de roupa suja (roupa de cama, banho e uniforme) enviado à lavanderia** em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de hotelaria**, incluindo lavanderia e manutenção de roupas, entre os centros com internação
        3. **Unidade de medida**: Quilogramas (Kg)
        
        **Instruções específicas:**
        - Inclua apenas roupa efetivamente utilizada e enviada à lavanderia no período de referência
        - Associe cada valor ao centro de custo que gerou a roupa (normalmente setores com internação)
        - Deixe em branco ou zero se não houver consumo de roupa para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 120 Kg de roupa suja
        - Enfermaria Pediátrica: 80 Kg de roupa suja
        - Centro Cirúrgico: 50 Kg de roupa suja (uniformes e toalhas)
        
        Conclusão do exemplo:
        No período analisado, o total de roupa suja será utilizado para calcular o **rateio proporcional dos custos de hotelaria**, garantindo que cada centro de custo com internação arque com sua participação nos serviços de lavanderia e manutenção.
        
        ⚠️ **Importante**: Registre apenas a roupa efetivamente utilizada e enviada à lavanderia no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Kg de Roupa Suja"
        ...
        if not df_resultado.empty:
            ...
            total_kg = df_resultado['Quantidade'].sum()
            ...
            with col3:
                st.metric("Total de Kg de Roupa Suja", total_kg)

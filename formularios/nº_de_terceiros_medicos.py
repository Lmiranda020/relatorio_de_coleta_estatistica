...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Terceiros (Médicos)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade de médicos terceirizados** que atuam na unidade
        2. **Objetivo**: Auxiliar no **rateio de custos** por profissionais médicos em cada centro de custo
        3. **Unidade de medida**: Número de pessoas
        
        **Instruções específicas:**
        - Inclua todos os médicos que atuam na unidade de forma terceirizada
        - Não inclua médicos próprios da unidade
        - Preencha cada centro de custo separadamente
        - Deixe em branco ou zero se não houver médicos terceirizados no centro de custo
        
        **Exemplos:**
        - UTI Adulto: 3 médicos terceirizados
        - Centro Cirúrgico: 2 médicos terceirizados
        - Ambulatório: 1 médico terceirizado
        
        Conclusão do exemplo:
        No período analisado, o total de médicos terceirizados somou 6 profissionais, distribuídos entre os centros de custo. Esses dados serão utilizados para calcular o rateio proporcional dos custos de pessoal médico entre os diferentes setores.
        
        ⚠️ **Importante**: Confirme que os números correspondem apenas a médicos terceirizados e ativos no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Terceiros (Médicos)"
        ...
        ponderacao = "Nº de Terceiros (Médicos)"

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Remoções", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade de remoções de pacientes** realizadas entre unidades ou serviços
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de transporte de pacientes** entre os centros de custo beneficiados
        3. **Unidade de medida**: Número de remoções
        
        **Instruções específicas:**
        - Inclua apenas remoções de pacientes **entre unidades, serviços ou para fora da unidade**
        - **Não** inclua deslocamentos internos de pacientes entre salas dentro de um mesmo setor
        - Associe cada remoção ao centro de custo que se beneficiou do transporte
        - Deixe em branco ou zero se não houver remoções para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 5 remoções de pacientes para exames em outro setor ou unidade
        - Centro Cirúrgico: 3 remoções de pacientes para internação em outro serviço
        - Ambulatório: 2 remoções de pacientes para atendimento externo
        
        Conclusão do exemplo:
        No período analisado, o total de remoções será utilizado para calcular o **rateio proporcional dos custos de transporte de pacientes** entre os centros de custo beneficiados.
        
        ⚠️ **Importante**: Registre apenas remoções efetivamente realizadas entre unidades ou serviços, não contabilize deslocamentos internos dentro do mesmo setor.
        """)
    ...
    try:
        ...
        if not df_resultado.empty:
            ...
            total_remocoes = df_resultado['Quantidade'].sum()
            ...
            with col3:
                st.metric("Total de Remoções", total_remocoes)

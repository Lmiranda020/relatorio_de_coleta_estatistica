...
def render_form(competencia):
    """
    Renderiza o formulário RC_Potência Instalada (kw/h)
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - RC_Potência Instalada (kW/h)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a potência instalada** em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de energia elétrica** para cada setor
        3. **Unidade de medida**: kW/h (quilowatts por hora)
        
        **Instruções específicas:**
        - Utilize os dados oficiais de medição ou projetos elétricos da unidade
        - Inclua toda a potência instalada, independentemente do uso atual
        - Preencha cada centro de custo separadamente
        - Deixe em branco ou zero se não houver potência instalada para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 120 kW/h
        - Centro Cirúrgico: 200 kW/h
        - Ambulatório: 80 kW/h
        
        Conclusão do exemplo:
        No período analisado, a potência total instalada somou 400 kW/h, distribuída entre UTI, Centro Cirúrgico e Ambulatório. Esses dados serão utilizados para calcular o **rateio proporcional dos custos de energia elétrica**, por exemplo, entre os diferentes setores da unidade.
        
        ⚠️ **Importante**: Certifique-se de que os valores correspondem à potência instalada real, conforme documentação técnica da unidade.
        """)
    ...
    try:
        ...
        nome_formulario = "Potência Instalada (kw/h)"
        ...
        ponderacao = "Potência Instalada (kw/h)"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Potência Instalada (kw/h)", total_quantidade)

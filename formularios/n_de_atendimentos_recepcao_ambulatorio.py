...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Atendimentos Recepção Ambulatório", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de atendimentos realizados na recepção do ambulatório**, associando cada atendimento ao centro de custo do paciente
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais da recepção do ambulatório** entre os centros que utilizaram o serviço
        3. **Unidade de medida**: Número de atendimentos (cada paciente atendido = 1 ocorrência)
        
        **Instruções específicas:**
        - Cada atendimento realizado na recepção do ambulatório deve ser contabilizado como **1 atendimento**
        ...
        **Exemplo de cálculo:**
        - Dia 05/06/2022:
            - Pacientes da Clínica Médica → 10 atendimentos
            - Pacientes da UTI → 3 atendimentos
        - Dia 06/06/2022:
            - Pacientes do Pronto-Socorro → 5 atendimentos
        - Total do mês:
            - Clínica Médica: 10 atendimentos
            - UTI: 3 atendimentos
            - Pronto-Socorro: 5 atendimentos
        
        Conclusão do exemplo:
        Os números registrados permitirão calcular o **rateio proporcional dos custos do setor de recepção do ambulatório** entre os centros de custo atendidos.
    ...
    try:
        ...
        nome_formulario = "N° de Atendimentos Recepção Ambulatório"
        ...
        ponderacao = "Nº de Atendimentos Recepção Ambulatório"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Atendimentos Recepção Ambulatório", total_quantidade)

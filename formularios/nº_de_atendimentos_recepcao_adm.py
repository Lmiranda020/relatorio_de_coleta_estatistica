...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Atendimentos Recepção ADM", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de pacientes atendidos na recepção administrativa**, associando cada atendimento ao centro de custo do paciente
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos do setor de recepção administrativa** entre os centros de custo que utilizaram o serviço
        3. **Unidade de medida**: Número de atendimentos (cada paciente atendido = 1 ocorrência)
        
        **Instruções específicas:**
        - Cada paciente atendido na recepção deve ser contabilizado como **1 atendimento**
        - Associe sempre o atendimento ao **centro de custo do paciente**
        - Se o mesmo paciente for atendido em dias diferentes, registre cada atendimento separadamente
        - Deixe em branco ou zero se não houver atendimentos para determinado centro de custo
        
        **Exemplo de cálculo:**
        - Dia 05/06/2022:
            - Paciente A (Clínica Médica) → 1 atendimento para **Clínica Médica**
            - Paciente B (Farmácia) → 1 atendimento para **Farmácia**
            - Paciente C (Clínica Médica) → 1 atendimento para **Clínica Médica**
        - Total do dia:
            - Clínica Médica: 2 atendimentos
            - Farmácia: 1 atendimento
        
        Conclusão do exemplo:
        Cada atendimento registrado será utilizado para calcular o **rateio proporcional dos custos do setor de recepção administrativa**, considerando o centro de custo real de cada paciente.
    ...
    try:
        ...
        nome_formulario = "Nº de Atendimentos Recepção ADM"
        ...
        ponderacao = "Nº de Atendimentos Recepção ADM"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Atendimentos Recepção ADM", total_quantidade)

...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - N° Peças (Hotelaria)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe a quantidade de peças enviadas à lavanderia** em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos de hotelaria**, incluindo lavanderia e manutenção de roupas, entre os centros que geraram as peças
        3. **Unidade de medida**: Número de peças
        
        **Instruções específicas:**
        - Inclua apenas peças efetivamente utilizadas e enviadas à lavanderia no período de referência
        - Associe cada valor ao centro de custo que gerou as peças
        - Deixe em branco ou zero se não houver envio de peças para o centro de custo
        
        **Exemplos:**
        - UTI Adulto: 50 peças enviadas
        - Enfermaria Pediátrica: 30 peças enviadas
        - Centro Cirúrgico: 20 peças enviadas (uniformes e toalhas)
        
        Conclusão do exemplo:
        No período analisado, o total de peças enviadas será utilizado para calcular o **rateio proporcional dos custos de hotelaria**, garantindo que cada centro de custo arque com sua participação nos serviços de lavanderia e manutenção.
        
        ⚠️ **Importante**: Registre apenas as peças efetivamente utilizadas e enviadas à lavanderia no período de referência.
        """)
    ...
    try:
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Peças", total_quantidade)

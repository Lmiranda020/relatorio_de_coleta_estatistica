...
def render_form(competencia):
    ...
    """
    # Caixa de instruções
    with st.expander("📋 Instruções - Nº de Atendimentos Recepção Pronto Socorro", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de atendimentos realizados na recepção do Pronto Socorro**, associando cada atendimento ao centro de custo do paciente
        2. **Objetivo**: Auxiliar no **rateio proporcional dos custos operacionais da recepção do Pronto Socorro** entre os centros que utilizaram o serviço
        3. **Unidade de medida**: Número de atendimentos (cada paciente atendido = 1 ocorrência)
        
        **Instruções específicas:**
        - Cada atendimento realizado na recepção do Pronto Socorro deve ser contabilizado como **1 atendimento**
        - Associe sempre o atendimento ao **centro de custo do paciente**
        - Se o mesmo paciente receber múltiplos atendimentos em dias diferentes, registre cada atendimento separadamente
        - Deixe em branco ou zero se não houver atendimentos para determinado centro de custo
        
        **Exemplo de cálculo:**
        - Dia 05/06/2022:
            - Pacientes da UTI → 4 atendimentos
            - Pacientes da Clínica Médica → 7 atendimentos
        - Dia 06/06/2022:
            - Pacientes do Pronto Socorro → 5 atendimentos
        - Total do mês:
            - UTI: 4 atendimentos
            - Clínica Médica: 7 atendimentos
            - Pronto Socorro: 5 atendimentos
        
        Conclusão do exemplo:
        Os números registrados permitirão calcular o **rateio proporcional dos custos do setor de recepção do Pronto Socorro** entre os centros de custo atendidos.
        
        ⚠️ **Importante**: Registre apenas os atendimentos efetivamente realizados no período de referência e associe corretamente ao centro de custo do paciente.
        """)
    ...
    try:
        ...
        nome_formulario = "N° de Atendimentos Recepção Pronto Socorro"
        ...
        ponderacao = "N° de Atendimentos Recepção Pronto Socorro"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de N° de Atendimentos Recepção Pronto Socorro", total_quantidade)

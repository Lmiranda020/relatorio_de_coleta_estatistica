...
def render_form(competencia):
    ...
    """
    # Nº de Itens Movimentados - Farmácia Satélite Centro Cirúrgico
    with st.expander("📋 Instruções - Nº de Itens Movimentados (Farmácia Satélite Centro Cirúrgico)", expanded=False):
        st.markdown("""
        ### Como preencher este formulário:
        
        1. **Informe o número de movimentações de itens realizadas pela Farmácia Satélite do Centro Cirúrgico** em cada centro de custo
        2. **Objetivo**: Auxiliar na **análise e rateio dos custos operacionais da Farmácia Satélite** entre os centros que se beneficiaram do serviço de dispensa
        3. **Unidade de medida**: Número de movimentações de itens (cada movimentação corresponde a uma requisição atendida, independentemente da quantidade de unidades)
        
        **Instruções específicas:**
        - Cada movimentação corresponde a uma requisição atendida pelo setor, mesmo que a requisição contenha múltiplos itens
        - Associe cada valor ao centro de custo que solicitou o atendimento
        - Deixe em branco ou zero se não houver movimentações para o centro de custo
        
        **Exemplo de cálculo:**
        - Dia 04/06/2022: 1 pedido de fentanil com 8 unidades e 1 pedido de midazolam com 5 unidades → **2 movimentações**
        - Dia 10/06/2022: 1 pedido de paracetamol com 10 unidades e 1 pedido de amoxicilina com 7 unidades → **2 movimentações**
        - Total do mês: 4 movimentações, **independente da quantidade de unidades solicitadas**
        
        Conclusão do exemplo:
        No período analisado, o total de movimentações será utilizado para calcular o **rateio proporcional dos custos operacionais da Farmácia Satélite** entre os diferentes centros de custo beneficiados.
        
        ⚠️ **Importante**: Registre apenas as movimentações efetivamente realizadas no período de referência.
        """)
    ...
    try:
        ...
        nome_formulario = "Nº de Itens Movimentados - Farmácia Satélite Centro Cirúrgico"
        ...
        ponderacao = "Nº de Itens Movimentados - Farmácia Satélite Centro Cirúrgico"
        ...
        if not df_resultado.empty:
            ...
            with col3:
                st.metric("Total de Nº de Itens Movimentados - Farmácia Satélite Centro Cirúrgico", total_quantidade)

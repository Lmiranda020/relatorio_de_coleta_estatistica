...
import pandas as pd
import json
from config.constants import DEPARA_PONDERACAO
from data.limpeza_base_de_para_poderacao import (
...
def de_para_ponderacao():
    """
    Consome a API para subir dados de centro de custo para UMA unidade específica.
    ...
    try:
        ...
        st.info(f"📊 Obtendo token da unidade...{unidade_id}")
        ...
        dados_depara_ponderacao = carregar_e_tratar_depara_json(DEPARA_PONDERACAO)
        
        if not dados_depara_ponderacao:
            st.error("❌ Nenhum dado de de-para foi carregado!")
            return False
        
        st.success(f"✅ {len(dados_depara_ponderacao)} registros preparados para envio")
        
        # Validar estrutura dos dados
        if not validar_estrutura_depara(dados_depara_ponderacao):
            st.error("❌ Estrutura dos dados inválida!")
            return False
        
        # Exibir estatísticas
        stats = obter_estatisticas_depara(dados_depara_ponderacao)
        ...
        if dados_depara_ponderacao and len(dados_depara_ponderacao) > 0:
            # Cria lista de mapeamentos apenas com os campos necessários para a API
            mapeamentos_limpos = []
            for item in dados_depara_ponderacao:
                mapeamento = {
                    "ponderacaoDeRateioErp": item.get("ponderacaoDeRateioErp", "").strip(),
                    "ponderacaoDeRateioKpih": item.get("ponderacaoDeRateioKpih", "").strip()
                }
                # Só adiciona se ambos os campos estão preenchidos
                if mapeamento["ponderacaoDeRateioErp"] and mapeamento["ponderacaoDeRateioKpih"]:
                    mapeamentos_limpos.append(mapeamento)
        else:
            st.warning("⚠️ Nenhum mapeamento encontrado, usando dados padrão")
            mapeamentos_limpos = []

        # Usa SEMPRE a competência do usuário
        ...
        with st.spinner(f"📡 Processando {nome_unidade} (ID: {unidade_id})..."):
            ...
            # URL da API
            url = f'https://backoffice.kpih.com.br:8000/api/v2/kpih/mapeamento/ponderacao_rateio'

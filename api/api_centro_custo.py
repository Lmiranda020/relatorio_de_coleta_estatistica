...
import pandas as pd
import json
from config.constants import DEPARA_EXCEL
...
def consumir_api_unidade_especifica():
    """
    ...
    try:
        ...
        dados_depara = carregar_e_tratar_depara_json(DEPARA_EXCEL)
        ...
        if dados_depara and len(dados_depara) > 0:
            ...
            for item in dados_depara:
                mapeamento = {
                    "centroDeCustoErp": item.get("centroDeCustoErp", "").strip(),
                    "centroDeCustoKpih": item.get("centroDeCustoKpih", "").strip()
                }
                # Só adiciona se ambos os campos estão preenchidos
                if mapeamento["centroDeCustoErp"] and mapeamento["centroDeCustoKpih"]:
        ...
        with st.spinner(f"📡 Processando {nome_unidade} (ID: {unidade_id})..."):
            ...
            # URL da API
            url = f'https://backoffice.kpih.com.br:8000/api/v2/kpih/mapeamento/centro_custo'
